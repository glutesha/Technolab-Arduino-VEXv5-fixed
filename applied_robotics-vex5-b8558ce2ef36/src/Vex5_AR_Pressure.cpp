...
int8_t Vex5_AR_Pressure::begin(VEX5_PORT_t port, Vex5_t* shield)
{
  this->shield = shield;
  int8_t result = -1;
  if((port > VEX5_PORT_NO) && (port <= VEX5_PORT_12))
  {
    ...
    if(ver == 5) {                        //Process port numbers for version 5
  	...
  	data[0] = this->port - 1;
  	while(shield->sendData((VEX5_DEVICE_ID_t)id, VEX5_CMD_AR_PRESSURE_BEGIN, data, 1))
      delay(50);
...
uint16_t res_press;
uint16_t Vex5_AR_Pressure::getValue()
{
  uint16_t result = 0;
  if (port)
  {
    ...
    shield->sendData((VEX5_DEVICE_ID_t)id, VEX5_CMD_AR_PRESSURE_GET_VAL, data, 1);
    int8_t len = shield->receiveData(data);
    if(len > 0)
    {
      result = 0;
